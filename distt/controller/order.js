...
export const createOrder = TryCatch(async (req, res, next) => {
    const { shippingCharges, user, total, subTotal, orderItems, discount, tax, shippingInfo, } = req.body;
...
export const myOrder = TryCatch(async (req, res, next) => {
    ...
    if (myNode.has(`myorders-${user}`)) {
        orders = JSON.parse(myNode.get(`myorders-${user}`));
    }
    else {
...
export const allOrder = TryCatch(async (req, res, next) => {
    ...
    if (myNode.has("all-orders")) {
        orders = JSON.parse(myNode.get("all-orders"));
    }
    else {
...
export const getSingleOrder = TryCatch(async (req, res, next) => {
    ...
    if (myNode.has(`order-${id}`)) {
        order = JSON.parse(myNode.get(`order-${id}`));
    }
    else {
