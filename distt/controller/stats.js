...
export const dashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myNode.has(key)) {
        ...
        const totalOrderRevenue = allOrders.reduce((total, order) => total + order.total, 0);
        const count = {
            orders: ordersCount,
...
export const pieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myNode.has(key)) {
        ...
        const discount = allOrders.reduce((discount, order) => discount + order.discount, 0);
        const grossIncome = allOrders.reduce((grossIncome, order) => grossIncome + order.total, 0);
        const burnt = allOrders.reduce((burnt, order) => burnt + order.tax, 0);
        const productionCost = allOrders.reduce((cost, order) => cost + order.shippingCharges, 0);
        const marketingCost = allOrders.reduce((cost, order) => cost + (order.total * 30) / 100, 0);
